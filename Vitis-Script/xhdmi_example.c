...
* and Video PHY Controller drivers along with VPSS and Video Mixer IP
*
* <pre>
...
/************************** Local Routines & Variable Definations -- VPSS ********************************/

static int setup_video_io(XPeriph *PeriphPtr, XVprocSs *VpssPtr, XVidC_VideoStream *HdmiTxSsVidStreamPtr);
...
XPeriph PeriphInst;
XVprocSs VprocInst;
u8 customCfmtFlag = FALSE ;
XVidC_ColorFormat customCfmt = XVIDC_CSF_NUM_SUPPORTED ;
...
// VMIX -- Layers and Display funtions.
static int EnableAndDisplay(int layerIndex) ;
...
#define XVMIX_LAYER_ADDR_OFFSET    (0x01500000U)

#define XVMIX_CHROMA_ADDR_OFFSET   (0x03000000U)
...
/**
*
* This function toggles the VPSS mode.  (PIP/ZOOM)
*
*
* @return None.
*
...
void Exdes_TogglePipZoom()
{
	...
	if(XVprocSs_IsPipModeOn(VpssPtr))
	{
		xil_printf("TOGGLE: Pip Mode to Zoom Mode \r\n Current Mode: ZOOM Mode \r\n\n") ; // 0
		XVprocSs_SetZoomMode(VpssPtr, TRUE);
	}
	else
	{
		xil_printf("TOGGLE: Zoom Mode to Pip Mode \r\n Current Mode: PIP Mode \r\n\n") ; // 0

		XVprocSs_SetPipMode(VpssPtr, TRUE);
	}
...
/**
*
* This function prints the VPSS operational window.
*
* @return None.
*
*
******************************************************************************/
void Exdes_GetStreamWindow()
{
	// VPSS in main()
	XVprocSs *VpssPtr;

	VpssPtr = &VprocInst;
	XVprocSs_GetZoomPipWindow(VpssPtr, XVPROCSS_PIP_WIN, &win);

//	xil_printf("XV_HscalerIs444Enabled: %d\r\n", XV_HscalerIs444Enabled(VpssPtr->HscalerPtr));
//	xil_printf("XV_HscalerIs420Enabled: %d\r\n", XV_HscalerIs422Enabled(VpssPtr->HscalerPtr));
//	xil_printf("XV_HscalerIs420Enabled: %d\r\n", XV_HscalerIs420Enabled(VpssPtr->HscalerPtr));

	xil_printf("Current Window Setting \r\n X: %d, Y: %d, width: %d, height: %d \r\n\n", 	win.StartX,	win.StartY, win.Width, win.Height ) ; // 0
}
...
/**
*
* This function prints the IN-OUT color format.
*
*
******************************************************************************/
void Exdes_GetIOColorFormat()
{
	XVidC_ColorFormat color_formats[] = {XVIDC_CSF_RGB, XVIDC_CSF_YCRCB_444, XVIDC_CSF_YCRCB_422, XVIDC_CSF_YCRCB_420} ;
...
//	VpssPtr->VidIn.ColorFormatId ;

    xil_printf("Input Color Format: %s\r\n\r\n",
                    XVidC_GetColorFormatStr(VpssPtr->VidIn.ColorFormatId));

    xil_printf("Output Color Format: %s\r\n\r\n",
                    XVidC_GetColorFormatStr(VpssPtr->VidOut.ColorFormatId ));
...
*
******************************************************************************/
void Exdes_SetBrightness(int val, int set)
{
	...
	if(set)
		XVprocSs_SetPictureBrightness(VpssPtr, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureBrightness(VpssPtr)) ;
...
/**
*
* This function updates the contrast
*
* @return None.
*
******************************************************************************/
void Exdes_SetContrast(int val, int set)
{
	...
	if(set)
		XVprocSs_SetPictureContrast(VpssPtr, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureContrast(VpssPtr)) ;
}
...
/**
*
* This function updates the color_range
*
* @return None.
*
******************************************************************************/
void Exdes_SetPictureColorRange(int val, int set)
{
	...
	if(set)
		XVprocSs_SetPictureColorRange(VpssPtr, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureColorRange(VpssPtr)) ;
}
...
/**
*
* This function updates the color standard
*
* @return None.
*
******************************************************************************/
void Exdes_SetPictureStandard(int val, int set)
{
	...
	if(set)
		XVprocSs_SetPictureColorStdOut(VpssPtr, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureColorStdOut(VpssPtr)) ;
}
...
/**
*
* This function updates the gain
*
* @return None.
*
******************************************************************************/
void Exdes_SetPictureGain(int val, int ch,  int set)
{
	...
	if(set)
		XVprocSs_SetPictureGain(VpssPtr, ch, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureGain(VpssPtr, ch)) ;
}

/*****************************************************************************/
/**
*
* This function updates the saturation
*
* @return None.
*
******************************************************************************/
void Exdex_SetSaturation(int val, int set)
{
	...
	if(set)
		XVprocSs_SetPictureSaturation(VpssPtr, val) ;
	else
		printf(" %d\r\n",XVprocSs_GetPictureSaturation(VpssPtr) ) ;
...
void Exdes_UpdateWindow()
{
	...
	// update zoom/pip window
	XVprocSs_UpdateZoomPipWindow(VpssPtr) ;
...
/**
*
* This function sets the VPSS PIP stream operational window
*
* @return None.
*
******************************************************************************/
void Exdes_SetPipStreamWindow(XVidC_VideoWindow input_win)
{
	// VPSS in main()
	XVprocSs *VpssPtr;
	VpssPtr = &VprocInst;

	if(validatWindowSize(&input_win, &VpssPtr->VidOut.Timing) == XST_FAILURE)
	{
		xil_printf("Can't change Window settings - invalid window configuration \r\n" ) ; // 0
	}
	else if(XVprocSs_IsPipModeOn(VpssPtr))
	{
		xil_printf("Can't change Window settings when PIP mode is on - Toggle PIP window \r\n" ) ; // 0
	}
	else
	{
		win = input_win ;
		XVprocSs_SetZoomPipWindow(VpssPtr, XVPROCSS_PIP_WIN, &win);

		xil_printf("Window Setting Changed \r\n X: %d, Y: %d, width: %d, height: %d \r\n", 	win.StartX,	win.StartY, win.Width, win.Height ) ; // 0
	}
...
/**
*
* This function updates the VPSS operational window for picture settings
*
* @return None.
*
******************************************************************************/
void Exdes_SetPictureDemoWindow(XVidC_VideoWindow input_win)
{
	...
	if(validatWindowSize(&input_win, &VpssPtr->VidOut.Timing) == XST_FAILURE)
	{
		xil_printf("ERROR - Invalid window configuration \r\n" ) ; // 0
		return ;
	}

	XVprocSs_SetPictureDemoWindow(VpssPtr, &input_win);
}
...
/**
*
* This function sets the VPSS ZOOM stream operational window
*
* @return None.
*
*
******************************************************************************/
void Exdes_SetZoomStreamWindow(XVidC_VideoWindow input_win)
{
	// VPSS in main()
	XVprocSs *VpssPtr;
	VpssPtr = &VprocInst;
	...
	{
		xil_printf("Can't change Window settings - invalid window configuration \r\n" ) ; // 0
		xil_printf("Window Setting \r\n X: %d, Y: %d, width: %d, height: %d \r\n", 	input_win.StartX,	input_win.StartY, input_win.Width, input_win.Height ) ; // 0

	}
	else if(XVprocSs_IsZoomModeOn(VpssPtr))
	{
		xil_printf("Can't change Window settings when Zoom mode is on - Toggle Zoom window \r\n" ) ; // 0
	}
	else
	{
		win = input_win ;
		XVprocSs_SetZoomPipWindow(VpssPtr, XVPROCSS_ZOOM_WIN, &win);
...
/**
*
* This function sets the Background color (VPSS)
*
* @return None.
*
******************************************************************************/
void Exdes_SetBGColor(XLboxColorId bg_color_id)
{
	// VPSS in main()
	XVprocSs *VpssPtr;
	VpssPtr = &VprocInst;

    XVprocSs_SetPIPBackgroundColor(VpssPtr, bg_color_id);
	xil_printf("Background Color Changed !!\r\n" ) ; // 0
...
/**
*
* This function enables Mixer
*
* @return None.
*
******************************************************************************/
void Exdes_EnableMixer(int layerIndex)
{

	XV_Mix_l2 *MixerPtr = &mix;

	// check if  the layer are enabled ; if not enable Mixer

	if(!XVMix_IsLayerEnabled(MixerPtr, layerIndex) )
		EnableAndDisplay(layerIndex) ;
	else
		xil_printf("Mixer Layer %d Already Enabled !\r\n", layerIndex) ;
...
/**
*
* This function disables Mixer
*
* @return None.

*
******************************************************************************/
void Exdes_DisableMixer(int layerIndex)
{
	...
	// check if all the layers are enabled ; if not enable Mixer

	if(!XVMix_IsLayerEnabled(MixerPtr, layerIndex))
		xil_printf("Mixer Layer %d already disabled !\r\n", layerIndex);
	else
	{
		if(XVMix_LayerDisable(MixerPtr, layerIndex) != XST_SUCCESS)
		{
			xil_printf("unable to disable layer\r\n Exiting \r\n");
					return ;
		}
		else
			xil_printf("Disabled layer %d ! \r\n", layerIndex);
...
/**
*
* This function set the alpha for a layer Mixer
*
* @return None.
*
*
******************************************************************************/
void Exdes_SetLayerAlpha(int layerIndex, int alpha_val)
{
	...
	if(layerIndex < 4)
	{
		xil_printf("   Setting Layer Alpha to %d: \r\n", alpha_val);
		if(XVMix_IsAlphaEnabled(MixerPtr, layerIndex)) {
		  Status = XVMix_SetLayerAlpha(MixerPtr, layerIndex, alpha_val);
		  if(Status != XST_SUCCESS) {
			xil_printf("<ERROR:: alpha Command Failed %ld>\r\n", Status);
		  } else {
			xil_printf("Done\r\n");
		  }
		} else {
	...
	{
		Status = XVMix_SetLayerAlpha(MixerPtr, XVMIX_LAYER_LOGO, XVMIX_ALPHA_MAX);
		if(Status != XST_SUCCESS) {
			xil_printf("<ERROR:: alpha Command Failed %ld>\r\n", Status);
		} else {
			xil_printf("Done\r\n");
...
/**
*
* This function scales a layer Mixer
*
* @return None.
*
*
******************************************************************************/
void Exdes_SetScaleFactor(int layerIndex, XVMix_Scalefactor scaleFactor)
{
	XV_Mix_l2 *MixerPtr = &mix;
	u32 Status ;

//     scale and move
    xil_printf("   Seting Layer Scale Factor to %d \r\n", scaleFactor);
...
//	xil_printf("%d\n%d\n%d\n", VidStreamPtr->FrameRate, VideoMode, ColorFormat);
...
TxInputSourceType Exdes_DetermineTxSrc()
{
	...
	// debug - print tx input source
	//	xil_printf("TxInputSrc = %d\r\n", TxInputSrc);
	return TxInputSrc;
}
#endif
#ifdef XPAR_XV_HDMITXSS1_NUM_INSTANCES
...
*
* 1) Program the VPSS input stage to the input video format.
...
static int setup_video_io(XPeriph *PeriphPtr, XVprocSs *VpssPtr, XVidC_VideoStream *HdmiTxSsVidStreamPtr) {
	...
	width  = HdmiTxSsVidStreamPtr->Timing.HActive;
	height = HdmiTxSsVidStreamPtr->Timing.VActive;
	resId = HdmiTxSsVidStreamPtr->VmId;
	FrameRate = HdmiTxSsVidStreamPtr->FrameRate ;
	cfmt = HdmiTxSsVidStreamPtr->ColorFormatId ;


	status = XSys_SetStreamParam(VpssPtr, XSYS_VPSS_STREAM_IN, width, height, FrameRate, cfmt, FALSE);

	if (status != XST_SUCCESS)
	{
		xil_printf("Failed to Setup Input Stream ! %d ", status);
	...
	*/
	...
	// based on customCfmtFlag - modify the output resolution and color format
	if(customCfmtFlag)
	{
		cfmt = customCfmt ;
//		HdmiTxSsVidStreamPtr->ColorFormatId = cfmt;
		XVidC_SetVideoStream(HdmiTxSsVidStreamPtr, HdmiTxSsVidStreamPtr->VmId, XVIDC_CSF_MEM_YUV8, 8, 4);
		XVidC_ReportStreamInfo(HdmiTxSsVidStreamPtr) ;
	}


	status = XSys_SetStreamParam(VpssPtr, XSYS_VPSS_STREAM_OUT, width, height, FrameRate, cfmt, FALSE);

	if (status != XST_SUCCESS)
	{
		xil_printf("Failed to Setup Output Stream !  %d ", status);
		return XST_FAILURE;
	}
...
//	xil_printf("\n\rStart VPSS...\r\n");
	...
	if (status != XST_SUCCESS)
	{
		xil_printf(" Failed to Start VPSS ! ");
		return XST_FAILURE;
	}
 ...
 *
 ******************************************************************************/
static void LoadVideoDataToMem()
{
  ...
  for(index=XVMIX_LAYER_1; index<NumLayers; ++index) {
	 ...
	 AlignBytes = 2 * MixerPtr->Mix.Config.PixPerClk * 4;
	 BytesPerPixel = ((CFmt == XVIDC_CSF_YCRCB_422) ? 2 : 4);
	 Stride = Win.Width * BytesPerPixel;
	 ...
	 if(index < XVMIX_LAYER_3) { //Load pre-defined images
	       ...
	       Stride = Win.Width * BytesPerPixel;
	       //  baseaddr
	       MemAddr = XVMix_GetLayerBufferAddr(MixerPtr, index);
	       dst = (char *)MemAddr;
	       for(y=0; y<Win.Height; ++y) {
	    	   for(x=0; x<Stride; ++x) {

	    		   dst[y*Stride+x] = ((index == 1) ? imagedata2[y*Stride+x]
												   : img2[y*Stride+x]);
	    	   }
	       }
  ...
  // set this flag to 1; to avoid writing into memory
  LoadVideoDataToMemFlag = TRUE ;
}
 ...
 *
 ******************************************************************************/
void ResetMixer(XGpio *ResetPtr, u32 Channel, u32 Reset)
{
...
/**
 *
 * This function reads the resets of the mixer block
 *
 * @param ResetPtr is ptr to reset block
 * @param channel is the GPIO channel id
 ...
 *
 ******************************************************************************/
u32 ReadResetMixer(XGpio *ResetPtr, u32 Channel)
{
  return XGpio_DiscreteRead(ResetPtr, Channel); //reset is active low
}
...
static int EnableAndDisplay(int layerIndex)
{
	...
	if(layerIndex < 4)
	{
		...
		Stride *= Win.Width; // stride = stirde * width
		...
		if(Status != XST_SUCCESS) {
			xil_printf("<ERROR:: Command Failed status %ld >\r\n", Status);
			++ErrorCount;
		} else {
			xil_printf("Done\r\n");
		}
		...
		if(Status != XST_SUCCESS) {
			xil_printf("<ERROR:: layer enable Command Failed %ld>\r\n", Status);
			++ErrorCount;
		} else {
			xil_printf("Done\r\n");
		}
	...
	else
	{
		  ...
		  if(XVMix_IsLogoEnabled(MixerPtr)) {
		    ...
		    xil_printf("logo layer enable Status: %d", Status) ;
		  } else {
		     xil_printf("  (Disabled in HW)\r\n");
		     return(ErrorCount);
		  }
		  ...
		  {
		      XVMix_LogoColorKey Data ={{10,10,10},{10,10,10}};
		  ...
		  xil_printf("LOGO ALPHA: %d\r\n", XVMix_GetLayerAlpha(MixerPtr, XVMIX_LAYER_LOGO)) ;;
			  Status = XVMix_SetLayerAlpha(MixerPtr, XVMIX_LAYER_LOGO, XVMIX_ALPHA_MAX);
			  if(Status != XST_SUCCESS) {
				xil_printf("<ERROR:: alpha Command Failed %ld>\r\n", Status);
				++ErrorCount;
...
//  xil_printf("XVMix_IsLogoEnabled: %d \r\n", XVMix_IsLogoEnabled(MixerPtr));

  // setup logo layer
  if(XVMix_IsLogoEnabled(MixerPtr)) {
    XVidC_VideoWindow Win;

    Win.StartX = (MixLayerConfig[2].StartX + MixLayerConfig[2].Width)/2;
    Win.StartY = (MixLayerConfig[2].StartY + MixLayerConfig[2].Height)/2;
    Win.Width  = 64;
    Win.Height = 64;
...
/* Place holder for future board support, Below Value just a random value */
	...
	if (Status != XST_SUCCESS) {
	xil_printf(
	"CRITICAL ERROR:: System Init Failed. Cannot recover from this error. Check HW\n\r");
	}
	...
	Status = XGpio_CfgInitialize(&MixerResetGpio,
							   GpioCfgPtr,
	...
	ResetMixer(&MixerResetGpio, MIXER_RESET_CH, FALSE);

	// read mixer reset here vmix
	xil_printf("Mixer Reset Value: %d\r\n", ReadResetMixer(&MixerResetGpio, MIXER_RESET_CH)) ;
	...
	if(Status != XST_SUCCESS) {
		xil_printf("ERROR:: V_MIX device not found\r\n");
	goto INFINITE_LOOP;
	};

	xil_printf("success V_MIX INIT... \r\n");
	...
	defined (XPAR_XV_HDMIRXSS1_NUM_INSTANCES)
			...
			ToggleHdmiRxHpd,
			Exdes_TogglePipZoom,
			Exdes_SetPipStreamWindow,
			Exdes_SetZoomStreamWindow,
			Exdes_GetStreamWindow,
			Exdes_SetBGColor,
			Exdes_EnableMixer,
			Exdes_DisableMixer,
			Exdes_SetLayerAlpha,
			Exdes_SetScaleFactor,
			Exdes_GetIOColorFormat,
			Exdes_UpdateWindow,
			Exdes_SetBrightness,
			Exdex_SetSaturation,
			Exdes_SetContrast,
			Exdes_SetPictureGain,
            Exdes_SetPictureStandard,
			Exdes_SetPictureColorRange,
			Exdes_SetPictureDemoWindow
			);
	...
	/* Start the system in pass-through mode. */ // in pass-through mode - force-independent is 0
	xhdmi_exdes_ctrlr.ForceIndependent = FALSE;
#else
	xhdmi_exdes_ctrlr.ForceIndependent = TRUE;
#if defined(XPAR_XV_HDMIRXSS1_NUM_INSTANCES)
	...
	defined (XPAR_XV_HDMIRXSS1_NUM_INSTANCES)
		...
		if (xhdmi_exdes_ctrlr.TxStartTransmit == TRUE && SinkReady) {
			...
			Status = setup_video_io(PeriphPtr, VpssPtr, HdmiTxSsVidStreamPtr);


			XVidC_VideoStream *VpssOut = &(VpssPtr->VidOut);

//			/* Toggle Mixer Reset */
			ResetMixer(&MixerResetGpio, MIXER_RESET_CH, TRUE);
			ResetMixer(&MixerResetGpio, MIXER_RESET_CH, FALSE);
//
			ConfigMixer(VpssOut) ;
			if(!LoadVideoDataToMemFlag)
				LoadVideoDataToMem() ;
