...
#endif
static XHdmi_MenuType XHdmi_MixerMenu(XHdmi_Menu *InstancePtr, u8 Input);
static XHdmi_MenuType XHdmi_SystemSetupMenu(XHdmi_Menu *InstancePtr, u8 Input);


static XHdmi_MenuType XHdmi_BGColorMenu(XHdmi_Menu *InstancePtr, u8 Input);
static XHdmi_MenuType XHdmi_PictureMenu(XHdmi_Menu *InstancePtr, u8 Input);
...
void XHdmi_MenuInitialize(XHdmi_Menu *InstancePtr, u32 UartBaseAddress,
		...
		void *ConfigureTpgEnableInputCB, void *ToggleHdmiRxHpdCB,  void *TogglePipZoomCB,void *SetPipStreamWindowCB,
		void *SetZoomStreamWindowCB, void *GetStreamWindowCB, void *SetBGColorCB, void *EnableMixerCB, void *DisableMixerCB,
		void *SetLayerAlphaCB, void *SetScaleFactorCB, void *GetIOColorFormatCB,  void *UpdateWindowCB,
		void *SetBrightnessCB, void *SetSaturationCB, void *SetContrastCB, void *SetPictureGainCB, void *SetPictureStandardCB,
		void *SetPictureColorRangeCB, void *SetPictureDemoWindowCB)
{
#ifdef XPAR_XV_HDMITXSS1_NUM_INSTANCES
#if(CUSTOM_RESOLUTION_ENABLE == 1)
    u32 Status;
    ...
    InstancePtr->ExDesCtrlr.ToggleHdmiRxHpdCB =
		ToggleHdmiRxHpdCB;
    InstancePtr->ExDesCtrlr.TogglePipZoomCB = TogglePipZoomCB;

    InstancePtr->ExDesCtrlr.TogglePipZoomCB = TogglePipZoomCB;
    InstancePtr->ExDesCtrlr.SetPipStreamWindowCB = SetPipStreamWindowCB;
    InstancePtr->ExDesCtrlr.SetZoomStreamWindowCB = SetZoomStreamWindowCB;
    InstancePtr->ExDesCtrlr.GetStreamWindowCB = GetStreamWindowCB;
    InstancePtr->ExDesCtrlr.SetBGColorCB = SetBGColorCB;
    InstancePtr->ExDesCtrlr.EnableMixerCB = EnableMixerCB;
    InstancePtr->ExDesCtrlr.DisableMixerCB = DisableMixerCB;
    InstancePtr->ExDesCtrlr.SetLayerAlphaCB = SetLayerAlphaCB;
    InstancePtr->ExDesCtrlr.SetScaleFactorCB = SetScaleFactorCB;
    InstancePtr->ExDesCtrlr.GetIOColorFormatCB = GetIOColorFormatCB;
    InstancePtr->ExDesCtrlr.UpdateWindowCB = UpdateWindowCB;
    InstancePtr->ExDesCtrlr.SetBrightnessCB = SetBrightnessCB;
    InstancePtr->ExDesCtrlr.SetContrastCB = SetContrastCB;
    InstancePtr->ExDesCtrlr.SetSaturationCB = SetSaturationCB;
    InstancePtr->ExDesCtrlr.SetPictureGainCB = SetPictureGainCB;
    InstancePtr->ExDesCtrlr.SetPictureStandardCB = SetPictureStandardCB;
    InstancePtr->ExDesCtrlr.SetPictureColorRangeCB = SetPictureColorRangeCB;
    InstancePtr->ExDesCtrlr.SetPictureDemoWindowCB = SetPictureDemoWindowCB;
...
void XHdmi_DisplayMainMenu(void)
{
    ...
    xil_printf("       => The Mixer Menu command provides options to modify the \r\n"
	       "          three preconfigured memory layers, a logo layer, and \r\n"
	       "          the main streaming layer.\r\n");
    xil_printf("v - System Setup(VPSS) Menu\r\n");
    xil_printf("       => The System Setup Menu command provides access to the\r\n"
    		"         VPSS features like Zoom and PIP, \r\n"
			"         and allows the window to be set for each feature independently.\r\n");
    xil_printf("t - TPG Menu\r\n");
    xil_printf("       => The Mixer Menu command provides options to modify the \r\n"
	       "          main streaming layer input to TPG.\r\n");
...
static XHdmi_MenuType XHdmi_MainMenu(XHdmi_Menu *InstancePtr, u8 Input) {
	...
	case ('M') :
		...
		Menu = XHDMI_MIXER_MENU;
		XHdmi_DisplayMixerMenu();

	    }

	    /* Pass-through */
	    else {
		xil_printf("The example design is in not pass-through mode.\r\n");
		xil_printf("In this mode the "
				"video mixer can't be enabled.\r\n");
	    }
	    break;
	...
	case ('V') :
	    ...
	    if (((*InstancePtr->ExDesCtrlr.IsRxPresentPtr) &&
			*InstancePtr->ExDesCtrlr.IsTxPresentPtr) ||
		(*InstancePtr->ExDesCtrlr.ForceIndependentPtr == FALSE)) {
		Menu = XHDMI_SYSTEMSETUP_MENU;
		XHdmi_DisplaySystemSetupMenu();
	    }
...
/**
*
* This function displays the BG Color Options.
*
* @param None
*
* @return None
...
*
******************************************************************************/
void XHdmi_DisplayBGColorMenu(void) {
    xil_printf("---------------------\r\n");
    xil_printf("---   Background Color Menu   ---\r\n");
    xil_printf("---------------------\r\n");
		xil_printf(" 1: Black\r\n");
		xil_printf(" 2: White\r\n");
		xil_printf(" 3: Red\r\n");
...
/**
*
* This function displays the Picture Menu .
*
* @param None
*
* @return None
...
*
******************************************************************************/
void XHdmi_DisplayPictureMenu(void) {
    xil_printf("---------------------\r\n");
    xil_printf("---   Picture Menu   ---\r\n");
    xil_printf("---------------------\r\n");
...
/**
*
* This function displays the VPSS menu.
*
* @param None
*
* @return None
...
*
******************************************************************************/
void XHdmi_DisplaySystemSetupMenu(void) {

    xil_printf("---------------------\r\n");
    xil_printf("---   VPSS Menu   ---\r\n");
    xil_printf("---------------------\r\n");

    xil_printf(" 1: Toggle ZOOM Mode\r\n");
	xil_printf(" 2: Toggle PIP Mode\r\n");
	xil_printf(" 3: Get Window \r\n");
	xil_printf(" 4: Set PIP Window \r\n");
	xil_printf(" 5: Set Zoom Window \r\n");
	xil_printf(" 6: Set Background \r\n");
	xil_printf(" 7: Get Input-Output Color Space Format \r\n");
	xil_printf(" 8: Show PIP/ZOOM Output \r\n");
	xil_printf(" 9: Modify Picture Menu \r\n");
	...
	/**
	*
	* This function displays the Video Mixer menu.
	*
	* @param None
	*
	* @return None
	...
	*
	******************************************************************************/
void XHdmi_DisplayMixerMenu(void) {

    xil_printf("---------------------\r\n");
    xil_printf("---   Mixer Menu   ---\r\n");
    xil_printf("---------------------\r\n");
    xil_printf(" 1: Enable Mixer Layers\r\n");
    xil_printf(" 2: Disable Mixer Layers\r\n");
...
/**
*
* This function implements the BG Colormenu state.
*
* @param input is the value used for the next menu state decoder.
*
* @return The next menu state.
...
//	 BACKGROUND COLORS
    ...
    /* Default */
    Menu = XHDMI_BGCOLOR_MENU;

    XHdmi_DisplayBGColorMenu() ;
    switch (Input) {
	case 1 :
		InstancePtr->ExDesCtrlr.SetBGColorCB(BKGNDCOL[0]);
	    Menu = XHDMI_SYSTEMSETUP_MENU;

		break;
	case 2 :
		InstancePtr->ExDesCtrlr.SetBGColorCB(BKGNDCOL[1]);
	    Menu = XHDMI_SYSTEMSETUP_MENU;
	...
	case 3 :
		...
		InstancePtr->ExDesCtrlr.SetBGColorCB(BKGNDCOL[2]);
	    Menu = XHDMI_SYSTEMSETUP_MENU;


		break;
	case 4 :
		InstancePtr->ExDesCtrlr.SetBGColorCB(BKGNDCOL[3]);
	    Menu = XHDMI_SYSTEMSETUP_MENU;


		break;
	case 5 :
		InstancePtr->ExDesCtrlr.SetBGColorCB(BKGNDCOL[4]);
	    Menu = XHDMI_SYSTEMSETUP_MENU;
...
/**
*
* This function implements the Video Mixer menu state.
*
* @param input is the value used for the next menu state decoder.
*
* @return The next menu state.
...
static XHdmi_MenuType XHdmi_PictureMenu(XHdmi_Menu *InstancePtr, u8 Input) {
	...
	case 1 :
		...
		xil_printf("Enter Brightness Value [0-100] | Current Brightness: ");
		InstancePtr->ExDesCtrlr.SetBrightnessCB(pic_value, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		InstancePtr->ExDesCtrlr.SetBrightnessCB(pic_value, 1);
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
		break;
	case 2 :
		// configure contrast
		xil_printf("Enter Contrast Value [0-100] | Current Contrast:");
		InstancePtr->ExDesCtrlr.SetContrastCB(pic_value, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		InstancePtr->ExDesCtrlr.SetContrastCB(pic_value, 1);
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
	...
	case 3 :
		// configure contrast
		xil_printf("Enter Saturation Value [0-100] | Current Saturation:");
		InstancePtr->ExDesCtrlr.SetSaturationCB(pic_value, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		// todo brightness fun
		InstancePtr->ExDesCtrlr.SetSaturationCB(pic_value, 1);
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
	...
	case 4 :
		...
		// configure contrast
		xil_printf("Enter Red Gain Value [0-100] | Current Red Gain:");
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_Y_RED, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		// todo brightness fun
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_Y_RED, 1) ;
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
		break;
	...
	case 5 :
		// todo set window
		// configure contrast
		xil_printf("Enter Green Gain Value [0-100] | Current Green Gain:");
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_CB_GREEN, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		// todo brightness fun
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_CB_GREEN, 1) ;
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
		break;
	case 6 :
		// todo set window
		// configure contrast
		xil_printf("Enter Blue Gain Value [0-100] | Current Blue Gain:");
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_CR_BLUE, 0) ;
		xil_printf("> ");

		pic_value = XHdmi_GetInput() ;

		// todo brightness fun
		InstancePtr->ExDesCtrlr.SetPictureGainCB(pic_value, XVPROCSS_COLOR_CH_CR_BLUE, 1) ;
		Menu = XHDMI_BGCOLOR_MENU;
		XHdmi_DisplaySystemSetupMenu();
		break;
	case 7 :
		// todo set window
		// configure contrast
		xil_printf("Enter Picture Standard 0: BT2020 1: BT709 2: BT601 3: [EXIT] | Current Standard :");
		InstancePtr->ExDesCtrlr.SetPictureStandardCB(pic_value, 0) ;
		xil_printf("> ");
		...
		// todo brightness fun
		if(pic_value < 3)
			InstancePtr->ExDesCtrlr.SetPictureStandardCB(pic_value, 1) ;
	...
	case 8 :
		// todo set window
		// configure contrast
		xil_printf("Enter Picture Color Range 0: CR_16_235 1: CR_16_240 2: CR_0_255 3: [EXIT] | Current Color Range :");
		InstancePtr->ExDesCtrlr.SetPictureColorRangeCB(pic_value, 0) ;
		xil_printf("> ");
		...
		// todo brightness fun
		if(pic_value < 3)
			InstancePtr->ExDesCtrlr.SetPictureColorRangeCB(pic_value, 1) ;
	...
	case 9:
		// todo set window
		// configure contrast
		xil_printf("Enter Demo Window: All the picture setting will apply only within defined window");

		xil_printf("Enter Window Start X: \r\n") ;
		xil_printf("> ");
		input_win.StartX = XHdmi_GetInput() ;

		xil_printf("Enter Window Start Y: \r\n") ;
		xil_printf("> ");
		input_win.StartY = XHdmi_GetInput() ;

		xil_printf("Enter Window Width: \r\n") ;
		xil_printf("> ");
		input_win.Width = XHdmi_GetInput() ;

		xil_printf("Enter Window Height: \r\n") ;
		xil_printf("> ");
		input_win.Height = XHdmi_GetInput() ;

		InstancePtr->ExDesCtrlr.SetPictureDemoWindowCB(input_win) ;
...
/**
*
* This function implements the Picture Settings menu state.
*
* @param input is the value used for the next menu state decoder.
*
* @return The next menu state.
...
static XHdmi_MenuType XHdmi_SystemSetupMenu(XHdmi_Menu *InstancePtr, u8 Input) {
	...
	case 1 :
		//  toggle zoom layer
//		xil_printf("XHdmi_SystemSetupMenu %d", Input) ;
		InstancePtr->ExDesCtrlr.TogglePipZoomCB() ;
		XHdmi_DisplaySystemSetupMenu() ;
	...
	case 3 :
		...
		InstancePtr->ExDesCtrlr.GetStreamWindowCB() ;
		XHdmi_DisplaySystemSetupMenu() ;
		break;
	case 4 :
		//  set zoom windows
		...
		xil_printf("Window Passed \r\n X: %d, Y: %d, width: %d, height: %d \r\n", 	input_win.StartX,	input_win.StartY, input_win.Width, input_win.Height ) ; // 0

		InstancePtr->ExDesCtrlr.SetPipStreamWindowCB(input_win) ;
		XHdmi_DisplaySystemSetupMenu();


		break;
	case 5 :
		// todo set zoom window
		...
		InstancePtr->ExDesCtrlr.SetZoomStreamWindowCB(input_win) ;
		XHdmi_DisplaySystemSetupMenu() ;
	...
	case 7 :
		...
		// display input-output color format

		InstancePtr->ExDesCtrlr.GetIOColorFormatCB() ;
		XHdmi_DisplaySystemSetupMenu();
//	    Menu = XHDMI_BGCOLOR_MENU;
...
//	    Menu = XHDMI_BGCOLOR_MENU;
	...
	case 9 :
		...
		XHdmi_DisplayPictureMenu();
	    Menu = XHDMI_PICTURE_MENU;
...
static XHdmi_MenuType XHdmi_MixerMenu(XHdmi_Menu *InstancePtr, u8 Input) {
	...
	case 1 :
		// todo enable layers
//		InstancePtr->ExDesCtrlr.EnableMixerCB() ;

		// todo -- new
		xil_printf("Enter Layer Index between 1 and 3 & [4] for LOGO Layer: \r\n") ;
		layer_ind = XHdmi_GetInput() ;
		if(layer_ind <1 || layer_ind >4)
		{
			xil_printf("Incorrect Layer Index... Try Again \r\n") ;
			break;
		}
		else if( layer_ind < 4)
			InstancePtr->ExDesCtrlr.EnableMixerCB(layer_ind) ;
		else
			InstancePtr->ExDesCtrlr.EnableMixerCB(XVMIX_LAYER_LOGO) ;
	...
	case 2 :
		// todo disable layers
//		InstancePtr->ExDesCtrlr.DisableMixerCB() ;
		...
		if(layer_ind <1 || layer_ind >4)
		{
			...
			InstancePtr->ExDesCtrlr.DisableMixerCB(layer_ind) ;
		else
			InstancePtr->ExDesCtrlr.DisableMixerCB(XVMIX_LAYER_LOGO) ;

		XHdmi_DisplayMixerMenu() ;
		break;
	case 3 :
		// todo set layer alpha
//		InstancePtr->ExDesCtrlr.SetLayerAlphaCB() ;

//		xil_printf("XHdmi_MixerMenu %d", Input) ;
		...
		xil_printf("Enter Alpha value between 0 and 256: \r\n") ;
		xil_printf("> ");
		alpha_val = XHdmi_GetInput() ;

		if(alpha_val <0 || alpha_val >256)
		{
			xil_printf("Incorrect Alpha value... Try Again \r\n") ;
			break;
		}

		if(layer_ind < 4)
			InstancePtr->ExDesCtrlr.SetLayerAlphaCB(layer_ind, alpha_val) ;
		else
			InstancePtr->ExDesCtrlr.SetLayerAlphaCB(XVMIX_LAYER_LOGO, alpha_val) ;
...
//		xil_printf("XHdmi_MixerMenu %d", Input) ;
		...
		xil_printf("Enter Scale Factor 1, 2 or 4: \r\n") ;
		xil_printf("> ");
		scale_factor = XHdmi_GetInput() ;

		if(scale_factor == 1)
		{

			if(layer_ind < 4)
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(layer_ind, 0) ;
			else
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(XVMIX_LAYER_LOGO, 0) ;
		...
		{
			if(layer_ind < 4)
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(layer_ind, 1) ;
			else
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(XVMIX_LAYER_LOGO, 1) ;
		}
		else if(scale_factor == 4)
		{
			if(layer_ind < 4)
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(layer_ind, 2) ;
			else
				InstancePtr->ExDesCtrlr.SetScaleFactorCB(XVMIX_LAYER_LOGO, 2) ;
		}
		else
		{
			xil_printf("Incorrect Scale Factor... Try Again \r\n") ;
			break;
		}
